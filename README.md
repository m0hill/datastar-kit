...
- `src/client.ts` — helpers for script tags and serving the self-hosted Datastar client asset.
- `src/response.ts` — Web `Response` helpers for empty command responses, Datastar SSE, and direct `text/html`, `application/json`, and `text/javascript` action responses.
...
- `src/datastar.ts` — typed signal/action/attribute/style helpers for generating/composing Datastar expressions and action URLs, validating signal names, merging attributes, inspector/ref helpers, object-form bindings, modifier suffixes, and `data-*` attributes.
