...
describe("Datastar modifier helpers", () => {
  ...
  it("builds prevent/stop and throttle modifiers", () => {
    expect(
      ds.on("submit", ds.post("/save"), {
        prevent: true,
        stop: true,
        throttle: { duration: "1s", trailing: true }
      })
    ).toEqual({
  ...
  it("builds intersection modifiers", () => {
    expect(
      ds.onIntersect(ds.expr("$visible = true"), { once: true, full: true, delay: 100 })
    ).toEqual({
  ...
  it("builds signal patch timing modifiers", () => {
    expect(
      ds.onSignalPatch(ds.expr("console.log(patch)"), {
        debounce: { duration: "250ms", noTrailing: true }
      })
    ).toEqual({
