...
const FormResult = ({ method, values }: { readonly method: string; readonly values: readonly string[] }) => (
  <output id="form-data-result" class="event-output">
    {values.length === 0
      ? `${method}: no values submitted`
      : `${method}: ${values.join(", ")}`}
  </output>
)
...
export const formDataExample: ExampleModule = {
  ...
  register(app) {
            ...
            <form id="form-data-demo" class="checks">
              ...
              <div role="group">
                <button
                  ...
                  {...ds.on(
                    "click",
                    ds.get("/examples/form_data/endpoint", { contentType: "form" })
                  )}
                >
                  Submit GET request
                </button>
                <button
                  ...
                  {...ds.on(
                    "click",
                    ds.post("/examples/form_data/endpoint", { contentType: "form" })
                  )}
                >
                  Submit POST request
                </button>
    ...
    app.get("/examples/form_data/endpoint", async (c) =>
      reply.patch(<FormResult method="GET" values={await valuesFromRequest(c.req.raw)} />)
    )

    app.post("/examples/form_data/endpoint", async (c) =>
