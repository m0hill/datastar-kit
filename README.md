...
- `src/platform.ts` — adapter from ts-star Web handlers to `@effect/platform` HTTP apps, so prototypes can run through Effect's `NodeHttpServer` instead of only raw Node plumbing.
