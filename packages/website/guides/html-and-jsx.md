...
For ordinary component replacement, prefer returning an element with a stable top-level `id` and omit `selector`. Use explicit selectors for container operations such as `inner`, `append`, `prepend`, or `remove`.
