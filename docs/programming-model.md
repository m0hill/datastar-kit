...
Application state lives in backend resources: databases, services, in-memory examples, durable queues, etc. Framework helpers may hold request/connection resources, subscriptions, and invalidation channels, but they should not hold authoritative UI state per browser connection.
...
4. The command returns:
   - `commandDone()` / `datastarNoContentResponse()` for successful mutations with no local feedback; or
   - a Datastar patch/direct response for validation errors, optimistic local feedback, or local-only UI changes.
...
- Decode untrusted request input with Effect Schema.
...
- Return `204` by default with `commandDone()`.
...
Queries read current state and render HTML. They can be normal request handlers or direct Datastar response handlers.

Use `currentViewPatchResponse(state, render, patchOptions)` when a handler already has current state and needs to patch an element.
...
- Use heartbeats/backpressure/cancellation at the runtime layer as those APIs mature.

`liveQuery({ invalidations, load, render, patch })` returns a stream of Datastar element patch events. `liveQueryResponse(...)` wraps it in a `200` Datastar SSE response.

Invalidation payloads may be `void`; they are just triggers. If a client disconnects and misses invalidations, reconnecting still renders current state.

## Action URLs and routing

Today, examples route commands and queries explicitly with Effect Platform `HttpRouter.route` and generate action URLs with Datastar helpers such as `post('/increment')` or `get(queryUrl('/search', { q }))`.
...
The live-query pattern should always render current state on connect, so reconnects are safe.

## Minimal API surface introduced for this model

`src/model.ts` adds small helpers rather than a full `Page` DSL:

- `commandDone(options?)` — returns a Datastar-safe `204` response.
- `currentViewPatchResponse(state, render, patchOptions?, responseOptions?)` — renders current state and returns a Datastar element patch response.
- `liveQuery({ invalidations, load, render, patch })` — creates a current-state Datastar event stream.
- `liveQueryResponse(options, responseOptions?)` — wraps `liveQuery` in a Datastar SSE response.

These helpers define the intended semantics while leaving the exact future `Page`, `Action`, and route DSL flexible.

## Example

`examples/live-counter.ts` is the first end-to-end CQRS example:
