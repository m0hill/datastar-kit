...
`ts-star` does not provide a public auth/session/CSRF subsystem. Those policies belong to the application router, middleware, or app-owned services.

A safe Datastar command should:

1. receive a native `Request` through the application framework;
2. decode Datastar signals with `read.signals(request, schema)` or use Web APIs/framework readers for other input;
